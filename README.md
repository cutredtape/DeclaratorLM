...
<p align="center">
  <img src="docs/screenshots/01-dashboard.png" alt="Головний екран DeclaratorLM: дашборд зі зведеною статистикою по всіх оброблених деклараціях" width="880">
</p>
...
Результат — звіт з усіма деклараціями особи в хронології, **графіками динаміки ризику, фінансів і майна по роках**, і LLM-резюме всього досьє:
...
- 🧠 **Прозорість** — можна вмикати стрімінг «думок» (reasoning) моделей і бачити хід міркувань.
...
| 🐍 Backend | Python 3.11+, майже без зовнішніх залежностей |
...
Перед першим запуском застосунку потрібно один раз зібрати фронтенд (або запустити `BUILD_FRONTEND.bat`):

```bash
cd declarator-lm
npm install
npm run build   # → declarator-lm/dist/ (для обох мовних версій одразу)
```
...
|----------|--------------|
| 🖥️ **Локально** (приватно, свій GPU) | Серія **Qwen 3.x, від 32B і більше** — на сьогодні найкращий баланс якість/приватність для цієї задачі. Менші моделі — лише «побавитися». |
| 🌐 **Хмара** (OpenRouter) | Сильні інструктивні моделі: Llama 3.3 70B, великі Qwen, тощо. Тут можна взяти потужнішу модель без вимог до власного заліза (але дані підуть провайдеру — див. [Приватність](#privacy)). |
...
| 🧠 **THINK_EVENT Debug** | Вмикає стрімінг «думок» reasoning-моделей — видно хід міркувань у реальному часі. |
...
- Reasoning debug: стрімінг «думок» моделі
